...
function getCollection(agenda) {
  if (!agenda)
    throw new Error('Agenda instance is not undefined');
  if (agenda.db) {
    return agenda.db.collection;
  }
  if (agenda._collection) {
    return agenda._collection.collection || agenda._collection;
  }
  throw new Error('this Agenda verison is unknown');
}
...
module.exports = function (agenda, options) {
  ...
  agenda.on("ready", () => {
    const collection = getCollection(agenda);
    ...
    if (agenda._mdb) {
      ...
      collection.client.db().command({ buildInfo: 1 })
      .then(info => {
        if (!semver.satisfies(semver.coerce(info.version), ">=3.6.0")) {
          throw new Error("MongoDB version not supported");
        }
      })
      .catch(error => {
